...
function OtherProfile() {
  ...
  const [rand, setRand] = useState(
    Math.floor(Math.random() * AllProfile.length + 1)
  );
  const dispatch = useDispatch();
  const navigate = useNavigate();
  useEffect(() => {
    setRand(Math.floor(Math.random() * AllProfile.length + 1));
  }, []);
  useEffect(() => {
    setRand(Math.floor(Math.random() * AllProfile.length + 1));
    ...
    <>
      <Card bg="dark" className="text-white rounded-3 text-wrap">
        ...
        {!AllProfile.length ? (
          ...
          <ListGroup>
            {AllProfile.slice(rand, rand + 4).map((p) => {
              return (
                <ListGroup.Item
                  key={p._id}
                ...
                >
                  ...
                  <div
                    className="text-white flex-column ps-3 pb-2 clickable"
                    onClick={() => navigate(`/profile/${p._id}`)}
                  >
                    <h5>
                      {p.name} {p.surname}
